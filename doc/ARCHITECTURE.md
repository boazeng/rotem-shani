...
## שכבת התרחישים (בתוך ה-HTML)

מודול ב-`rotem-shani.html` (חפש `🎯 SCENARIOS`) שבונה **תוכנית תנועה רב-שלבית** מעל מנוע המשימות ומודד את הזמנים בפועל. שני חיבורים בלבד למנוע הקיים:

1. **שעון** — `SR.clock += dt` בראש `stepScenario`. ה-`dt` כבר מוכפל ב-`CFG.speed`, ולכן השעון מודד **זמן מתקן** ולא זמן צפייה.
2. **`onIdleHook`** — בנקודה שבה `stepScenario` כבר מזהה סוף-ריצה (`running` והכל ריק), נקרא ה-hook במקום ה-`flash`. זו נקודת "זמן הסיום" של פעולה.

`scenDispatch()` נקרא בסוף כל `stepScenario`: משחרר אירועים שהגיע זמנם לתור, ומתחיל את הבא **רק כש-`busy()` שקר** — כי למתקן יש מעלית אחת. פירוט מלא ב-[SCENARIOS.md](SCENARIOS.md).
